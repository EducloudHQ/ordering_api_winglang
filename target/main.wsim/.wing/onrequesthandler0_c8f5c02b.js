...
exports.handler = async function(event) {
  $handler = $handler ?? (
              ...
              require("/home/fonchu/Desktop/winlang/learn/target/main.wsim/.wing/inflight.$Closure4-1.js")({
                $__parent_this_4_counter: (function() {
  let handle = process.env.COUNTER_HANDLE_444dce03;
  if (!handle) {
    throw new Error("Missing environment variable: COUNTER_HANDLE_444dce03");
  }
  ...
  return require("@winglang/sdk/lib/simulator/client").makeSimulatorClient(simulatorUrl, handle);
        ...
        ,
                $queue: (function() {
  let handle = process.env.QUEUE_HANDLE_25e0ce1c;
  if (!handle) {
    throw new Error("Missing environment variable: QUEUE_HANDLE_25e0ce1c");
  }
  const simulatorUrl = process.env.WING_SIMULATOR_URL;
  if (!simulatorUrl) {
    throw new Error("Missing environment variable: WING_SIMULATOR_URL");
